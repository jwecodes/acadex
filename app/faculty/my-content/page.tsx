...
import { FileText, Download, Trash2, CheckCircle, Clock, AlertCircle, XCircle } from 'lucide-react'
...
export default function MyContentPage() {
  ...
  useEffect(() => {
    ...
    if (!facultyId) {
      console.log('No faculty ID, redirecting to login')
      router.push('/faculty/login')
      return
    }
  ...
  const loadContent = async (facultyId: string) => {
    try {
      console.log('Fetching /api/faculty/my-submissions with facultyId:', facultyId)
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white shadow-sm border-b mb-8">
        <div className="max-w-7xl mx-auto px-6 py-6">
          ...
          <p className="text-gray-600 mt-2">Track your uploaded teaching materials and approval status</p>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 pb-8">
        {error && (
          <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
            ...
            <button
              onClick={() => {
                const facultyId = localStorage.getItem('facultyId')
                if (facultyId) loadContent(facultyId)
              }}
              className="text-red-600 hover:text-red-700 mt-2 text-sm underline"
            >
              Try again
            </button>
          </div>
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-8">
          ...
          <p className="text-sm text-gray-600">
            Showing <span className="font-semibold text-blue-600">{filteredContents.length}</span> of{' '}
            <span className="font-semibold">{contents.length}</span> items
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow">
          {filteredContents.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <p className="text-lg font-medium text-gray-600 mb-2">No content found</p>
              <Link href="/faculty/dashboard" className="text-blue-600 hover:text-blue-700 inline-block">
                Go to dashboard
              </Link>
            </div>
          ) : (
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-2 text-sm text-gray-600 mb-3">
                          <span>{content.course?.courseCode || 'N/A'}</span>
                          {content.course?.courseCode && <span>•</span>}
                          <span>{(content.contentType || '').replace(/_/g, ' ')}</span>
                          <span>•</span>
                          <span>
                        ...
                        {content.coordinatorNotes && (
                          <div className="mt-3 p-3 bg-orange-50 border border-orange-200 rounded">
                            ...
                            <p className="text-sm text-orange-800">{content.coordinatorNotes}</p>
                          </div>
                        )}
                      </div>
